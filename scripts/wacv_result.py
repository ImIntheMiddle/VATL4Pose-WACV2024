...
def load_result_json(result_dir, strategy_list, video_id_list, result_dict, sc_thresh):
    ...
    for strategy in strategy_list:
        ...
        result_dict[strategy] = {"Percentage": {}, "mean_uncertainty": {}, "combine_weight": {}, "spearmanr": {}, "actual_finish": {}, "finished_minerror": {}, "finished_oursc": {}, "stopped_AP_min": {}, "stopped_AP_oursc": {}, "OSPA_1000": {}, "OSPA_ALC": {}, "mOKS_list": {}}
        for video_id in video_id_list:
            ...
            if os.path.exists(result_files[-1]):
              ...
              OSPA_list_ann = result["ospa_ann"]
              OSPA_1000 = interpolate.interp1d(result["percentages"], OSPA_list_ann)
              result_dict[strategy]["OSPA_1000"][video_id] = OSPA_1000(percent1000)
              result_dict[strategy]["OSPA_ALC"][video_id] = compute_alc(result["percentages"], OSPA_list_ann)
    ...
    if "THC+WPU_Coresetfilter" in result_dict.keys():
      print(result_dict["_K-Meansfilter"]["AP .6"+"_ALC_ann"])
      print(result_dict["THC+WPU_Coresetfilter"]["AP .6"+"_ALC_ann"])
      for i in result_dict["_K-Meansfilter"]["AP .6"+"_ALC_ann"].keys():
        if result_dict["_K-Meansfilter"]["AP .6"+"_ALC_ann"][i] < result_dict["THC+WPU_Coresetfilter"]["AP .6"+"_ALC_ann"][i]:
          print(i + "won")
        else:
          print(i + "lose")
...
def summarize_result(save_dir, result_dict, metric, ANN=False):
  ...
  for strategy in result_dict.keys():
      ...
      metric_dict[strategy]["OSPA_ALC"] = float(np.mean(np.array(list(result_dict[strategy]["OSPA_ALC"].values()))))
      OSPA1000_mean = np.mean(np.array(list(result_dict[strategy]["OSPA_1000"].values())), axis=0)
      comberted_OSPA = np.array(OSPA1000_mean.tolist())
      metric_dict[strategy]["OSPA"] = comberted_OSPA[QUERY_RATIO].tolist()
  print("Done!\n")
  return metric_dict
...
def main(video_id_list_path, strategy_list, sc_thresh, RAW=False, ANN=True):
  ...
  for exp_name in strategy_list.keys():
    ...
    result_dir = os.path.join(root_dir, f"AL_{exp_name}", "FastPose")
